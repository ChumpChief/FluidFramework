...
module.exports = {
	...
	resolve: {
		...
		// `assert` is large when polyfilled in browser-facing JS. Setting its fallback to
		// `false` makes webpack fail the build (with the import chain) if anything pulls
		// it in. Use the assert API in @fluidframework/common-utils instead.
