...
export class BlobManager {
	...
	public get allBlobsAttached(): boolean {
		return [...this.localBlobCache.values()].every((blob) => blob.state !== "localOnly");
	}

	// TODO: Probably get rid of this
	public get hasPendingBlobs(): boolean {
		return [...this.localBlobCache.values()].some((blob) => blob.state !== "attached");
	}

	public hasBlob(blobId: string): boolean {
		return this.redirectTable.has(blobId) || this.localBlobCache.has(blobId);
	}
